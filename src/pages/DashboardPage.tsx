...
export default function DashboardPage() {
  ...
  const saveEdit = (task: Task) => {
    dispatch(updateTask({ ...task, title: editTitle, description: editDescription }))
    setEditingId(null)
  }
            ...
            {items.map((t) => (
              <li key={t.id} className="card p-6 flex flex-col gap-3 shadow-lg transition-transform hover:scale-[1.03] hover:shadow-2xl border-2 border-transparent hover:border-fuchsia-200 animate-fadein">
                <div className="flex items-center justify-between gap-2">
                  {editingId === t.id ? (
                    <input
                      className="input font-semibold text-lg text-gray-800 truncate max-w-[70%]"
                      value={editTitle}
                ...
                {editingId === t.id ? (
                  <textarea
                    className="input text-sm mb-2 break-words whitespace-pre-line"
                ...
                <div className="flex items-center gap-2 mt-auto">
                  {editingId === t.id ? (
                    <>
                      <button
                        className="btn bg-gradient-to-r from-green-500 to-emerald-500 hover:from-emerald-500 hover:to-green-500 px-3 py-1 text-xs shadow"
                        onClick={() => saveEdit(t)}
                        type="button"
                      >
                        Save
                      ...
                      <button
                        className="btn bg-gradient-to-r from-gray-400 to-gray-600 hover:from-gray-600 hover:to-gray-400 px-3 py-1 text-xs shadow"
                        onClick={cancelEdit}
                        type="button"
                      >
                        Cancel
                    ...
                    <>
                      ...
                      <button
                        className="btn bg-gradient-to-r from-yellow-400 to-fuchsia-500 hover:from-fuchsia-500 hover:to-yellow-400 px-3 py-1 text-xs shadow"
                        onClick={() => startEdit(t)}
                        type="button"
                      >
                        Edit
                      ...
                      <button
                        className="btn ml-auto bg-gradient-to-r from-pink-500 to-fuchsia-500 hover:from-fuchsia-500 hover:to-pink-500 transition-colors px-3 py-1 text-xs shadow"
